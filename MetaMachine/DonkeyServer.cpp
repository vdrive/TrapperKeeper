...
DonkeyServer::DonkeyServer(const char* ip,unsigned short port)
{
	m_ip=ip;
	m_port=port;
	m_tcp_handle=0;
...
DonkeyServer::~DonkeyServer(void)
{
}

bool DonkeyServer::IsServer(const char* ip)
{
...
void DonkeyServer::Init(void)
{
	m_connected=CTime(1971,1,1,1,1,1);
	TRACE("MetaMachine:  DonkeyServer::Init() %s %d\n",m_ip.c_str(),m_port);
	srand(timeGetTime());
	...
	for(UINT i=0;i<packet.GetLength();i++){
		if(isalnum(data[i])){
			TRACE("DonkeyServer::Init sent[%d] \t = \t 0x%x \t %c \n",i,data[i],data[i]);
		}
		else{//0x%x
			TRACE("DonkeyServer::Init sent[%d] \t = \t 0x%x \n",i,data[i]);
		}
	}*/
}

void DonkeyServer::NewConnection(UINT handle)
{
	if(m_tcp_firewall_check){
		TRACE("MetaMachine:  DonkeyServer::NewConnection()  We already have a connection???  This isn't right.\n");
		TCPReference tcp;
...
void DonkeyServer::NewData(UINT handle, Buffer2000 &data)
{
	if(handle==m_tcp_handle){
		CSingleLock sl(&m_tcp_buffer_lock,TRUE);
		if(m_state==this->S_INIT || m_state==this->S_INIT_RESPONSE){
			MetaMachineDll::GetDlg()->ServerLog(m_ip.c_str(),data);
		}
...
//REGULAR DATA
void DonkeyServer::ProcessMainBuffer(void)
{
...
//FIREWALL DATA
void DonkeyServer::ProcessFirewallBuffer(void)
{
...
void DonkeyServer::Reset(bool b_change_state)
{
	m_connected=CTime(1971,1,1,1,1,1);
	if(m_tcp_firewall_check){
		TCPReference tcp;
...
void DonkeyServer::Search(SearchJob *new_search)
{
	mv_search_tasks.Add(new_search);
}

void DonkeyServer::LostConnection(UINT handle)
{
	if(handle==m_tcp_handle){
		CString log_msg;
		//log_msg.Format("LOST CONNECTION TO %s!\n",m_ip.c_str());
		//MetaMachineDll::GetDlg()->Log((LPCSTR)log_msg);
		//TRACE("MetaMachine:  DonkeyServer::LostConnection() %s LOST it's MAIN CONNECTION!  This can't be good.\n",m_ip.c_str());
		m_tcp_handle=NULL;
		ChangeState(S_INIT);
		m_retry=0;
	}
	else if(handle==m_tcp_firewall_check){
		//TRACE("MetaMachine:  DonkeyServer::LostConnection() %s lost it's firewall back check!  This can't stop us now.\n",m_ip.c_str());
		m_tcp_firewall_check=NULL;
	}
}

void DonkeyServer::Update(void)
{
	for(UINT i=0;i<mv_search_tasks.Size();i++){
		...
		if(sj->IsClosed()){
			...
			TRACE("MetaMachine: DonkeyServer::Update() Removing closed search job from DonkeyServer %s.\n",m_ip.c_str());
	...
	if(m_state==S_INIT){
		if(( m_connect_retry<3 && m_state_time.HasTimedOut(40)) || m_state_time.HasTimedOut(1200)){  //let it wait a bit before trying to reconnect
			m_retry=0;
			m_connect_retry++;
			//TRACE("MetaMachine:  DonkeyServer %s going into INIT_RESPONSE  mode.\n",m_ip.c_str());
			Init();
	...
	if(m_state==S_INIT_RESPONSE){  //we are waiting for a connection response from this server
		if(m_state_time.HasTimedOut(60)){  //once every 30 seconds we will try to reconnect
			//TRACE("MetaMachine:  DonkeyServer %s init timed out, going back into INIT  mode.\n",m_ip.c_str());
			ChangeState(S_INIT);
		}
		return;
	}
	...
	if(m_state==S_READY_PAUSE){
		if(m_state_time.HasTimedOut(20)){  //allow a bit of a pause before we begin searching again
			//TRACE("MetaMachine:  DonkeyServer %s ready delay has completed, going to mode S_READY to do searches.\n",m_ip.c_str());
			ChangeState(S_READY);
			m_connect_retry=0;
		}
	}
	...
	if(m_state==S_READY){  //we are ready to do a search
		if(mv_search_tasks.Size()>0){
			m_retry=0;
			//TRACE("MetaMachine:  DonkeyServer %s going into search mode.\n",m_ip.c_str());
			mp_current_search_job=(SearchJob*)mv_search_tasks.Get(0);
			//TRACE("DonkeyServer %s beginning search for project %s.\n",m_ip.c_str(),mp_current_search_job->GetProject());
			CString log_msg;
			log_msg.Format("DonkeyServer %s beginning search for project %s.\n",m_ip.c_str(),mp_current_search_job->GetProject());
			MetaMachineDll::GetDlg()->Log((LPCSTR)log_msg);
	...
	if(m_state==S_SEARCH){
		//TRACE("MetaMachine:  DonkeyServer %s searching, going back into SEARCH_RESPONSE  mode.\n",m_ip.c_str());
	...
	if(m_state==S_SEARCH_RESPONSE){
		if(m_state_time.HasTimedOut(50)){  //give it 60 seconds before we just open a new connection and move onto another search
			...
			TRACE("MetaMachine:  DonkeyServer %s search response timed out, going into S_INIT mode.\n",m_ip.c_str());
	...
	if(m_state==S_HASH_STROBE){
		TRACE("MetaMachine:  DonkeyServer %s querying hash %d.\n",m_ip.c_str(),m_hash_probe);
	...
	if(m_state==S_HASH_STROBE_RESPONSE){
		if(m_state_time.HasTimedOut(20)){ 
			TRACE("MetaMachine:  DonkeyServer %s timed out on S_HASH_STROBE, going back to state S_HASH_STROBE.\n",m_ip.c_str());
			ChangeState(S_HASH_STROBE);
			m_retry++;
			if((UINT)m_retry>=mv_current_search_results.Size()){  //have we failed many times?
				AdvanceSearch();
				//TRACE("MetaMachine:  DonkeyServer %s timed out on S_HASH_STROBE, it has failed too many timed and is dropping back to state S_INIT.\n",m_ip.c_str());
	...
	if(m_state==S_HASH_STROBE_PAUSE){  //if in this state, we have gotten at least
		if(m_state_time.HasTimedOut(6)){  //take a break before strobing the next hash
			TRACE("MetaMachine:  DonkeyServer %s S_HASH_STROBE_PAUSE has completed, going into state S_HASH_STROBE to strobe %d.\n",m_ip.c_str(),m_hash_probe);
			ChangeState(S_HASH_STROBE);
		}	
		return;
	}
...
void DonkeyServer::SubmitSearch(void)
{
...
void DonkeyServer::QueryHash(void)
{
	SearchJob *search_job=mp_current_search_job;
	...
	if(sr==NULL){  //we are done.
		...
		mp_current_search_job=NULL;
//		TRACE("MetaMachine:  DonkeyServer %s
...
void DonkeyServer::ChangeState(SERVERSTATE new_state)
{
	...
	if(m_state==S_INIT){
		Reset(false);
		//TRACE("MetaMachine:  DonkeyServer %s dropping back to S_INIT state from %d state.\n",m_ip.c_str(),old_state);
	}
...
bool DonkeyServer::HasFullResponse(byte code)
{
...
void DonkeyServer::ProcessSearchResults(void)
{
	m_hash_probe=0;
...
//				b_ok=false;
	...
	//TRACE("MetaMachine:  DonkeyServer::ProcessSearchResults() length of tcp buffer after search is %d, length of results was %d.\n",m_tcp_buffer_post.GetLength(),len);
}
...
void DonkeyServer::ProcessHashQueryResponse(void)
{
	/*
...
void DonkeyServer::ClearUnwantedPackets(void)
{
	if(m_tcp_buffer_post.GetLength()<6)
		return;
	...
	while(index<m_tcp_buffer_post.GetLength()){
		...
		if(p_data[index+5]==0x38){  //is there a message that isn't the correct code?
			...
			//TRACE("MetaMachine:  DonkeyServer %s removing unwanted server message (0x38) from its buffer.  New buffer length is %d.\n",m_ip.c_str(),m_tcp_buffer_post.GetLength());
		}
...
void DonkeyServer::AdvanceSearch(void)
{
...
CTime DonkeyServer::ConnectionTime(void)
{
	return m_connected;
}

UINT DonkeyServer::Run(){
	while(!this->b_killThread){
		...
		if(b_new_normal_data){
			...
			if(m_state==S_INIT_RESPONSE){
				...
				CString log_msg;
				ChangeState(S_READY_PAUSE);
				//TRACE("MetaMachine:  DonkeyServer %s going into S_READY_PAUSE for search mode.\n",m_ip.c_str());
...
bool DonkeyServer::IsThreadLockedUp(void)
{
