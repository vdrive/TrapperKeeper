...
string PostData(const char *url, string referer, string post_info)
{
	...
	int size = PostData(url, &buf, referer, post_info);
	char * data = new char[size+1];
	for(int i = 0; i < size; i++) {
		data[i] = (char)buf[i];
	}
	data[size] = 0;
	string return_str = string(data);
	delete data;
	return return_str;
}

int PostData(const char * url, unsigned short **body, string referer, string header, const char *post_data, const int post_data_len) 
{
	...
	try {
		...
		char * cookie_name = "cookie";
		CString cookie_data;
		session.SetCookie(url, cookie_name, cookie_data); // bool
		session.GetCookie(url, cookie_name, cookie_data); // bool
		DWORD cookie_len = session.GetCookieLength(url, cookie_name);
		...
		pFile->SendRequest(header.c_str(),(DWORD)header.length(),(LPVOID)post_data, (DWORD)post_data_len );
