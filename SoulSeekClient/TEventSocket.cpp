...
//
//
int TEventSocket::Attach(SOCKET hSocket)
{
	int ret=0;
	ret=WEventSocket::Attach(hSocket);
	if(ret!=0)
	{
		return ret;
	}
	return TSocket::Attach(hSocket);	// not really necessary, but I just like being thorough :)
}
