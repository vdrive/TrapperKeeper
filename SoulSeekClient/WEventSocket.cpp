...
int WEventSocket::Create(unsigned short int port,unsigned int ip)
{
	...
	if((ret!=INVALID_SOCKET)&&(ret!=SOCKET_ERROR))
	{
		...
		ret=EventSelect(FD_CONNECT|FD_READ|FD_WRITE|FD_CLOSE);	// was FD_ALL_EVENTS
	}
...
//
//
int WEventSocket::Connect(char *host,unsigned short int port)
{
...
//
//
int WEventSocket::Attach(SOCKET hSocket)
{
	WSocket::Attach(hSocket);

	// Do the event select crap
	return EventSelect(FD_CONNECT|FD_READ|FD_WRITE|FD_CLOSE);	// was FD_ALL_EVENTS
}

//
//
//
SOCKET WEventSocket::Detach()
{
	EventSelect(0);	// turn off all event notifications
...
bool WEventSocket::OnReceive(int error_code)
{
	return false;
}
