...
int WAsyncSocket::Create(unsigned short int port,unsigned int ip)
{
	...
	if((ret!=INVALID_SOCKET)&&(ret!=SOCKET_ERROR))
	{
		...
		ret=AsyncSelect(FD_CONNECT|FD_READ|FD_WRITE|FD_CLOSE);	// was FD_ALL_EVENTS
	}
...
//
//
int WAsyncSocket::Attach(SOCKET hSocket)
{
	...
	// Do the asyncselect crap
	return AsyncSelect(FD_CONNECT|FD_READ|FD_WRITE|FD_CLOSE);	// was FD_ALL_EVENTS
}
...
void WAsyncSocket::SocketMessage(WPARAM wparam,LPARAM lparam)
{
	...
	switch(event_code)
	{
		...
		case FD_READ:
		{
			while(OnReceive(error_code));	// continute to call OnReceive() until it returns false
			break;
		}
		case FD_WRITE:
		{
...
bool WAsyncSocket::OnReceive(int error_code)
{
	return false;
}
