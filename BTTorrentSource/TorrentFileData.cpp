...
//		v_files.push_back(file);
	...
	m_torrent_id = *((unsigned int *)ptr);
	ptr+=sizeof(unsigned int);

	return (int)(ptr-bufbegin);
}

TorrentFileData & TorrentFileData::operator=(const TorrentFileData &data) 
{
	for (int i = 0; i < data.v_files.size(); i++)
	{
		this->v_files.push_back(data.v_files[i]);

	}
	this->m_name = data.m_name;
	this->m_piece_length = data.m_piece_length;

	for (i = 0; i < data.v_piece_hashes.size(); i++)
	{
		this->v_piece_hashes.push_back(data.v_piece_hashes[i]);

	}
	this->m_announce_URL = data.m_announce_URL;
	for (i = 0; i < data.v_announce_list.size(); i++)
	{
		this->v_announce_list.push_back(data.v_announce_list[i]);

	}
	this->m_creation_date = data.m_creation_date;
	this->m_comment = data.m_comment;
	this->m_created_by = data.m_created_by;
	this->m_hash = data.m_hash;
	this->m_torrent_id = data.m_torrent_id;
	return *this;
}