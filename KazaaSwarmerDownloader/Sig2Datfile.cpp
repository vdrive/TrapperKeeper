...
Sig2DatFile::Sig2DatFile(void)
{
	m_datState = 0;
	m_datSourceCount = 1;
	m_metaTagCount = 1;
//	m_datStartTime = GetCurrentTime();
	m_datStartTime = 0x3F7B57CD;
	m_datUnknown1_1 = 0xFFFFFFFF;
	m_datUnknown1_2 = 0x00000000;
	memset(m_datLocalFilePath,0,257);
	m_datUnknownTime= 0x00000000;
	m_datUnknown2 = 1;
	m_datCompletedChunks = 0;
	m_datFullRange1 = 0xFFFFFFFF;
	m_datFullRange2 = 0;
	m_datRangeShift = 0x01;
	m_datRangeState1 = 0x00000000;
	m_datRangeState2 = 0x00000000;
	m_datAppendixSignature = 0x415a414b;
...
bool Sig2DatFile::SetFilename(char * filename)
{
	//Get Location from Registry Instead later
	strcpy(m_datLocalFilePath,"C:\\FastTrack Shared\\");
	strcat(m_datLocalFilePath,filename);

	strcpy(m_datSourceList.m_srcRemoteFileName,filename);

	return true;
}

bool Sig2DatFile::SetHash(char * hash)
{
	char new_hash[20];
	memcpy(new_hash,hash,20);
	
	memcpy(m_datSourceList.m_srcContentHash,new_hash,20);
	memcpy(m_datMetaTag.m_tagValue,new_hash,20);

	return true;
}

bool Sig2DatFile::SetLength(int length)
{

	m_datSourceList.m_srcFileSize = length;
	return true;
}

string Sig2DatFile::WriteFile()
{
	string datfilename_return;
	DWORD Checksum = 0xFFFFFFFF;
	unsigned char sigfile[2056];
	int i = 1;

	// copy Each piece of Data to the stream
	sigfile[0] = 0x00;

	memcpy(&sigfile[i],&m_datState,4);
	i += 4;
	memcpy(&sigfile[i],&m_datSourceCount,4);
	i += 4;


	// Write the Source List Info
	memcpy(&sigfile[i],&m_datSourceList.m_srcRemoteFileName,(strlen(m_datSourceList.m_srcRemoteFileName)+1));
	i += ((int)strlen(m_datSourceList.m_srcRemoteFileName)+1);
	memcpy(&sigfile[i],&m_datSourceList.m_srcDownloadURL,1);
	i += 1;
	memcpy(&sigfile[i],&m_datSourceList.m_srcFileID,4);
	i += 4;
	memcpy(&sigfile[i],&m_datSourceList.m_srcContentHash,20);
	i += 20;
	memcpy(&sigfile[i],&m_datSourceList.m_srcFileSize,4);
	i += 4;
	memcpy(&sigfile[i],&m_datSourceList.m_srcNodeInetNum,4);
	i += 4;
	memcpy(&sigfile[i],&m_datSourceList.m_srcNodeTcpPort,4);
	i += 4;
	memcpy(&sigfile[i],&m_datSourceList.m_srcName,69);
	i += 69;
	memcpy(&sigfile[i],&m_datSourceList.m_srcKbps,4);
	i += 4;
	memcpy(&sigfile[i],&m_datSourceList.m_srcKbpsLastModified,4);
	i += 4;
	memcpy(&sigfile[i],&m_datSourceList.m_srcUnknown0,4);
	i += 4;

	// Adding 2 more unknowns for padding because I don't know where the 
	// extra 2 DWORDS are coming from.
	memcpy(&sigfile[i],&m_datSourceList.m_srcUnknown1,4);
	i += 4;
	memcpy(&sigfile[i],&m_datSourceList.m_srcUnknown1,4);
	i += 4;

	memcpy(&sigfile[i],&m_datSourceList.m_srcGroup,4);
	i += 4;
	memcpy(&sigfile[i],&m_datSourceList.m_srcRetry,4);
	i += 4;
	memcpy(&sigfile[i],&m_datSourceList.m_srcUnknown2,4);
	i += 4;
	memcpy(&sigfile[i],&m_datSourceList.m_srcUnknown3,1);
	i += 1;
	memcpy(&sigfile[i],&m_datSourceList.m_srcUnknown4,1);
	i += 1;

	// Write the Meta Tag info
	memcpy(&sigfile[i],&m_metaTagCount,4);
	i += 4;
	memcpy(&sigfile[i],&m_datMetaTag,m_datMetaTag.GetSize());
	i += sizeof(m_datMetaTag);


	// Write the rest
	memcpy(&sigfile[i],&m_datStartTime,4);
	i += 4;
	memcpy(&sigfile[i],&m_datUnknown1_1,4);
	i += 4;
	memcpy(&sigfile[i],&m_datUnknown1_2,4);
	i += 4;
	memcpy(&sigfile[i],&m_datLocalFilePath,(strlen(m_datLocalFilePath)+1));
	i += ((int)strlen(m_datLocalFilePath)+1);
	memcpy(&sigfile[i],&m_datUnknownTime,4);
	i += 4;
	memcpy(&sigfile[i],&m_datUnknown2,4);
	i += 4;
	memcpy(&sigfile[i],&m_datCompletedChunks,4);
	i += 4;

	// Something fishy in the order here between the K++ datfiles and the assumed format

	memcpy(&sigfile[i],&m_datFullRange1,4);
	i += 4;
	memcpy(&sigfile[i],&m_datRangeShift,1);
	i += 1;
	memcpy(&sigfile[i],&m_datFullRange2,4);
	i += 4;
	memcpy(&sigfile[i],&m_datRangeState1,4);
	i += 4;
	memcpy(&sigfile[i],&m_datRangeState2,4);
	i += 4;


	m_datAppendixSize = (i-1); // remove 1 for the 0 byte we added at the beginning
	// this stuff is not included in the checksum but is the end of the file
	//m_datAppendixChecksum = 0x8C3F45E7;
	// Do the Checksum of everything up to this point;
	Checksum = hashSmallHash(&sigfile[1], (i-1), Checksum);
//	Checksum = CalculateChecksum(sigfile, 1, i-1);

	m_datAppendixChecksum = Checksum;
	memcpy(&sigfile[i],&m_datAppendixSignature,4);
	i += 4;
	memcpy(&sigfile[i],&m_datAppendixSize,2);
	i += 2;


	memcpy(&sigfile[i],&m_datAppendixChecksum,4);
	i += 4;
			
	char	datfilename[256+1]; 
	strcpy(datfilename,"download-Swarmerdat");

	int len = (int)strlen(datfilename);
	for (int j = len; j < (len+20); j++)
	{
		_itoa(rand()%10,&datfilename[j],10);
	...
	CFile outputfile;
	if (outputfile.Open(datfilename,CFile::modeCreate|CFile::modeWrite|CFile::typeBinary)== FALSE)
	{
		DWORD damnerror = ::GetLastError();
		return false;
	}
	outputfile.Write(sigfile,i);
	outputfile.Close();

	datfilename_return = datfilename;

	return datfilename_return;
}


// Modified from Sig2Dat F them don't need their crap
/*
DWORD Sig2DatFile::CalculateChecksum(unsigned char* lpBuffer, DWORD dwBufferStart, DWORD dwBufferSize)
{
   DWORD lpBufferi;
   DWORD dwTemp;
   DWORD dwLookup;
   ...
   unsigned char Table[] = {0x00, 0x00, 0x00, 0x00, 0x96, 0x30, 0x07, 0x77, 0x2c, 0x61,
   ...
   dwChkSum = 0xFFFFFFFF;
   for (lpBufferi = dwBufferStart; 
        lpBufferi < dwBufferStart + dwBufferSize; 
        lpBufferi++)
      {
      dwLookup = ((dwChkSum  ^ lpBuffer[lpBufferi]) & 0xff) * 4;
      dwTemp = ((Table[dwLookup+3]*256 
              + Table[dwLookup+2])*256 
              + Table[dwLookup+1])*256 
              + Table[dwLookup+0];
      dwChkSum = (dwChkSum >> 8) ^ dwTemp;
	  if (dwChkSum == 0x8C3F45E7)
	  {
			int stop = 1;
	  }
      }
