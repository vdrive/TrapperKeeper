...
#ifdef _DEBUG
			if(g_db_supply_interface.OpenRawDataConnection("206.161.141.47","onsystems","ebertsux37","winmx_raw_supply"))
#else
			if(g_db_supply_interface.OpenRawDataConnection("localhost","onsystems","ebertsux37","winmx_raw_supply"))
#endif
...
void ProcessManager::DBMaintenance(void)
{
	if(!m_is_maintaining)
	{
		...
		while(!connected)
		{
			//db connection to raw data database
#ifdef _DEBUG
			connected = g_db_supply_interface.OpenRawDataConnection("206.161.141.47","onsystems","ebertsux37","winmx_raw_supply");
#else			
			connected = g_db_supply_interface.OpenRawDataConnection("localhost","onsystems","ebertsux37","winmx_raw_supply");
#endif
			trial++;
			if(trial > 10)
				break;
