...
//
int WSocket2::Create(unsigned int port)
{
	...
	if(ret==SOCKET_ERROR)
	{
		int error=WSocket2::GetLastError();
		return SOCKET_ERROR;
	}
...
//
//
int WSocket2::Connect(char *host,unsigned int port)
{
...
//
//
bool WSocket2::Attach(SOCKET hSocket)
{
...
//
int WSocket2::SendData(void *buf,unsigned int buf_len)
{
...
//
//
int WSocket2::SendQueuedDataBuffers()
{
	int ret;
...
//
//
int WSocket2::SendNextDataBuffer()
{
...
//
int WSocket2::Send(void *buf,unsigned int buf_len)
{
	WSABUF wsabuf;
	wsabuf.buf=(char *)buf;
	wsabuf.len=buf_len;
...
//
void WSocket2::OnSend(int error_code)
{
	if(m_sending_data==false)
	{
		return;
...
//
//
void WSocket2::DataSent(unsigned int len){}
...
//
int WSocket2::ReceiveData(unsigned int buf_len)
{
...
//
//
int WSocket2::ContinueToReceiveData()
{
	return ReceiveBuffer();
}
...
//
//
int WSocket2::Receive(void *buf,unsigned int buf_len)
{
	WSABUF wsabuf;
	wsabuf.buf=(char *)buf;
	wsabuf.len=buf_len;
...
//
void WSocket2::DataReceived(unsigned int len)
{
	// Free memory
	if(p_read_data_buf!=NULL)
	{
