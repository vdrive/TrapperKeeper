...
#include "WinMxDcDll.h"	// temp kludge for logging
	...
	while(1)
	{
		...
		if(event==1)
		{
			...
			*/
			...
			if(thread_data.m_stop_searching_project.GetLength() > 0)
			{
				sockets.StopSearchingProject(thread_data.m_stop_searching_project);
				thread_data.m_stop_searching_project.Empty();
			}
		}
...
void ConnectionModule::ConnectToHosts(vector<SupernodeHost> &hosts)
{
	...
	if(singleLock.IsLocked())
	{
		...
		sort(v_connect_to_ips.begin(),v_connect_to_ips.end());	// Tell the thread to connect to these hosts
...
//
//
bool ConnectionModule::IsConnected(SupernodeHost& host)
{
	if(p_critical_section==NULL)
	{
		return false;
	...
	if(singleLock.IsLocked())
	{
		...
		if(binary_search(v_connect_to_ips.begin(),v_connect_to_ips.end(),host.m_ip))
		{
			singleLock.Unlock();
			return true;
		}

		hash_set<SupernodeHost>::const_iterator iter;
		iter = hs_connected_host_ips.find(host);
		if(iter != hs_connected_host_ips.end())
		{
			singleLock.Unlock();
			return true;
		}
...
//
//
void ConnectionModule::StopSearchingProject(const char* project_name)
{
	if(p_critical_section==NULL)
	{
		return;
	...
	if(singleLock.IsLocked())
	{
		p_thread_data->m_stop_searching_project = project_name;
		WSASetEvent(p_thread_data->m_reserved_events[1]);	// vector data
		singleLock.Unlock();
	}
}
...
void ConnectionModule::StatusReady(WPARAM wparam,LPARAM lparam)
{
	...
	if(singleLock.IsLocked())
	{
		...
		hs_connected_host_ips.clear();
		for(UINT i=0; i<p_thread_data->p_status_data->v_connected_host_ips.size();i++)
		{
			hs_connected_host_ips.insert(p_thread_data->p_status_data->v_connected_host_ips[i]);
		}
...
{
	if(hs_connected_host_ips.size() >= 27)
		return true;
	else
		return false;
}
...
//
//
void ConnectionModule::GetConnectedHosts(vector<SupernodeHost>& hosts)
{
	if(p_critical_section==NULL)
	{
		return;
	...
	if(singleLock.IsLocked())
	{
		hash_set<SupernodeHost>::const_iterator iter = hs_connected_host_ips.begin();
		while(iter!=hs_connected_host_ips.end())
		{
