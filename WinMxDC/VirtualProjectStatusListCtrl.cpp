...
//
//
bool VirtualProjectStatusListCtrl::IncrementCounters(ProjectStatus &status)
{
	UINT i;
	...
	for(i=0;i<v_data.size();i++)
	{
		if(strcmp(status.ProjectName(),v_data[i].m_project_name.c_str())==0)
		{
			...
			if(v_data[i].m_query_hit_count > 100000)
				return true; //stop searching this project if we hit over 100k results
			else
				return false;
