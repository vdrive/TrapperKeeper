...
KeywordManager::KeywordManager()
{
	...
	for(UINT i=0; i<v_all_project_keywords.size();i++)
	{
		...
		{
			if(v_all_project_keywords[i].m_winmx_supply_enabled)
			{
...
#else
#ifdef WINMX_SWARMER
			if(v_all_project_keywords[i].m_winmx_swarming_enabled)
#else
#ifdef WINMX_SPOOFER
			if(v_all_project_keywords[i].m_winmx_spoofing_enabled)
#endif
#endif
#endif
			{
...
void KeywordManager::CreateProjectSupplyTable()
{
	DBInterface db;
#ifdef _DEBUG
	if(db.OpenSupplyConnection("206.161.141.47","onsystems", "ebertsux37", "winmx_raw_supply"))
#else
	if(db.OpenSupplyConnection("127.0.0.1","onsystems", "ebertsux37", "winmx_raw_supply"))
#endif
	{
		for(UINT i=0; i<v_project_keywords.size(); i++)
		{
