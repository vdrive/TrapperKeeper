...
int ProcessDBInterface::GetLastProcessedDateForSupply(CString project, char* date)
{
	...
	CString query_str = "SELECT MAX(date) from winmx_supply where project='";
	query_str += project;
	query_str += "'";
...
bool ProcessDBInterface::ProcessSupply(CString project, char* start, char* end, char* on_date, int total_tracks)
{
	...
	*/
	...
	vector<TrackSupply> total_supplies;
	for(i=0; i<track_supplies.size();i++)
	{
		bool found = false;
		for(j=0; j<total_supplies.size();j++)
		{
			if(track_supplies[i].m_track == total_supplies[j].m_track)
			{
				found = true;
				break;
			}
		...
		if(!found)
			total_supplies.push_back(track_supplies[i]);
		else
			total_supplies[j].m_supply += track_supplies[i].m_supply;
	}

	for(i=0; i<total_supplies.size();i++)
	{
		//if(search_ids.size() > 1)
		//	average_supplies[i].m_supply /= (int)search_ids.size();
		char track[32];
		char supply[32];
		sprintf(track, "%d", total_supplies[i].m_track);
		sprintf(supply, "%u", total_supplies[i].m_supply);
...
int ProcessDBInterface::InsertProcessedSupplyData(CString project, char* date, const char* track, const char* supply)
{
	...
	CString query_str = "INSERT INTO winmx_supply";
