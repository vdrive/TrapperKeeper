...
//
//
void NoiseManager::AcceptedConnection(vector<SOCKET>& sockets)
{
	for(UINT j=0; j<sockets.size();j++)
	{
		UINT i;
		...
		for(i=0;i<v_mods.size();i++)
		{
			...
			{	
				v_mods[i]->AcceptConnection(sockets[j]);
				found_mod=true;
				break;
			}
		}
		if(!found_mod)
		{
			...
			mod->AcceptConnection(sockets[j]);
		}
	}
}
