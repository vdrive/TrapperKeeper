...
bool  TrackerConnection::GetPeers(TorrentFileData * tf, string title) 
{
...
//				Log(string("BTDataCollector::GetPeers -- Tracker error: ") + string(temp), message_type);
				...
				delete [] temp;
				temp = NULL;
				// if it failed set the size of the peer list to -1
				size = -1;
			}
			delete [] trackerURL;
...
char * TrackerConnection::GetTrackerURL(TorrentFileData * tf, const char *url, char * peer_id) 
{
	...
	strcat(trackerURL, "0");
	//strcat(trackerURL, "&event=completed");
