...
*/
	...
	m_gift_spoofing_enabled=false;
	m_gift_decoys_enabled=false;
	m_gift_supply_enabled=false;
	m_winmx_supply_enabled=false;
	m_winmx_decoy_enabled=false;
	m_winmx_spoofing_enabled=false;
	m_winmx_swarming_enabled=false;
	m_kazaa_swarm_load=25;
	m_soulseek_supply_enabled=false;
	m_soulseek_demand_enabled=false;
	m_soulseek_spoofing_enabled=false;
	m_soulseek_decoying_enabled=false;
	m_bittorrent_decoy_enabled=false;
	m_bittorrent_interdiction_enabled=false;
	m_bittorrent_spoofing_enabled=false;
	m_bittorrent_supply_enabled=false;
	m_overnet_demand_enabled=false;
	m_overnet_supply_enabled=false;
	m_overnet_decoys_enabled=false;
	m_overnet_spoofing_enabled=false;
	m_gnutella_load=25;
}
...
*/	
	...
	if(m_version >= 0x00000004)
	{
		len += sizeof(m_gift_spoofing_enabled);
		len += sizeof(m_gift_decoys_enabled);
		len += sizeof(m_gift_supply_enabled);
	}
	if(m_version >= 0x00000005)
	{
		len += sizeof(m_kazaa_swarm_load);
	}
	if(m_version >= 0x00000006)
	{
		len += (sizeof(m_winmx_supply_enabled))*4;
	}
	if(m_version >= 0x00000007)
	{
		len += (sizeof(m_soulseek_supply_enabled))*4;
	}
	if(m_version >= 0x00000008)
	{
		len += (sizeof(m_bittorrent_decoy_enabled))*4;
	}
	if(m_version >= 0x00000009)
	{
		len += (sizeof(m_overnet_supply_enabled))*4;
	}
	if(m_version >= 0x0000000a)
	{
		len += sizeof(m_gnutella_load);
	}
	return len;
}
...
int ProjectKeywords::WriteToBuffer(char *buf)
{
	...
	*((unsigned int *)ptr)=(int)v_tracks_hash_counts.size();
	...
	if(m_version >= 0x00000004)
	{
		*((bool *)ptr) = m_gift_spoofing_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_gift_decoys_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_gift_supply_enabled;
		ptr += sizeof(bool);
	}

	if(m_version >= 0x00000005)
	{
		*((int*)ptr)=m_kazaa_swarm_load;
		ptr += sizeof(int);
	}
	if(m_version >= 0x00000006)
	{
		*((bool *)ptr) = m_winmx_supply_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_winmx_decoy_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_winmx_spoofing_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_winmx_swarming_enabled;
		ptr += sizeof(bool);
	}
	if(m_version >= 0x00000007)
	{
		*((bool *)ptr) = m_soulseek_supply_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_soulseek_demand_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_soulseek_spoofing_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_soulseek_decoying_enabled;
		ptr += sizeof(bool);
	}
	if(m_version >= 0x00000008)
	{
		*((bool *)ptr) = m_bittorrent_decoy_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_bittorrent_interdiction_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_bittorrent_spoofing_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_bittorrent_supply_enabled;
		ptr += sizeof(bool);
	}	
	if(m_version >= 0x00000009)
	{
		*((bool *)ptr) = m_overnet_demand_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_overnet_supply_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_overnet_decoys_enabled;
		ptr += sizeof(bool);
		*((bool *)ptr) = m_overnet_spoofing_enabled;
		ptr += sizeof(bool);
	}	
	if(m_version >= 0x0000000a)
	{
		*((int*)ptr)=m_gnutella_load;
		ptr += sizeof(int);
	}
...
int ProjectKeywords::ReadFromBuffer(char *buf)
{
	...
	if(m_version >= 0x00000004)
	{
		m_gift_spoofing_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_gift_decoys_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_gift_supply_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
	}
	if(m_version >= 0x00000005)
	{
		m_kazaa_swarm_load=*((int*)ptr);
		ptr+=sizeof(int);
	}
	if(m_version >= 0x00000006)
	{
		m_winmx_supply_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_winmx_decoy_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_winmx_spoofing_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_winmx_swarming_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
	}
	if(m_version >= 0x00000007)
	{
		m_soulseek_supply_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_soulseek_demand_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_soulseek_spoofing_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_soulseek_decoying_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
	}
	if(m_version >= 0x00000008)
	{
		m_bittorrent_decoy_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_bittorrent_interdiction_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_bittorrent_spoofing_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_bittorrent_supply_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
	}
	if(m_version >= 0x00000009)
	{
		m_overnet_demand_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_overnet_supply_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_overnet_decoys_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
		m_overnet_spoofing_enabled=*((bool *)ptr);
		ptr+=sizeof(bool);
	}
	if(m_version >= 0x0000000a)
	{
		m_gnutella_load=*((int*)ptr);
		ptr+=sizeof(int);
	}
	return GetBufferLength();
}
