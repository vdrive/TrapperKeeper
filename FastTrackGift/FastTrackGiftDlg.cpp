...
BEGIN_MESSAGE_MAP(CFastTrackGiftDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_SAVE_REPORT_BUTTON, OnBnClickedSaveReportButton)
	...
	ON_MESSAGE(WM_FILE_SHARING_MANAGER_UPDATE_SUPPLY_PROJECTS,UpdateFileSharingManagerSupplyProjects)
	ON_MESSAGE(WM_FILE_SHARING_MANAGER_THREAD_PROGRESS,FileSharingManagerThreadProgress)
	ON_MESSAGE(WM_FILE_SHARING_MANAGER_THREAD_PROJECT,FileSharingManagerThreadProject)
	ON_MESSAGE(WM_FILE_SHARING_MANAGER_SUPPLY_RETREIVAL_FAILED,FileSharingManagerSupplyRetrievalFailed)
...
BOOL CFastTrackGiftDlg::OnInitDialog()
{
	...
	//SetTimer(7,60*1000,0); //for writing supplies from us to db
...
//
//
void CFastTrackGiftDlg::KeywordsUpdated(vector<ProjectKeywords>* keywords)
{
	UINT i,j,k;
...
//
//
void CFastTrackGiftDlg::OnBnClickedResetProjectStatusList()
{
...
//
//
void CFastTrackGiftDlg::ClearProjectStatus(string &project)
{
...
//
//
void CFastTrackGiftDlg::OnDeltaposSupplyIntervalMultiplierSpin(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
...
//
//
void CFastTrackGiftDlg::AlterSupplyIntervalMultiplier(int dmulti)
{
...
//
//
void CFastTrackGiftDlg::OnBnClickedSaveReportButton()
{
	m_project_status_list.SaveReport();
}

//
//
//
LRESULT CFastTrackGiftDlg::InitDBManagerSupplyThreadData(WPARAM wparam,LPARAM lparam)
{
...
//
//
LRESULT CFastTrackGiftDlg::ReadyToWriteDataToSupplyDatabase(WPARAM wparam,LPARAM lparam)
{
...
//
//
void CFastTrackGiftDlg::OnBnClickedRestartSearchingButton()
{
...
//
//
LRESULT CFastTrackGiftDlg::SupplyProcessProject(WPARAM wparam,LPARAM lparam)
{
...
//
//
void CFastTrackGiftDlg::UpdateDataProcessProgressBar(int supply)
{
...
//
//
LRESULT CFastTrackGiftDlg::ProcessManagerMaintenanceDone(WPARAM wparam,LPARAM lparam)
{
...
//
//
LRESULT CFastTrackGiftDlg::InitProcessManagerMaintenanceThreadData(WPARAM wparam,LPARAM lparam)
{
...
//
//
LRESULT CFastTrackGiftDlg::ProcessManagerSupplyDone(WPARAM wparam,LPARAM lparam)
{
...
//
//
LRESULT CFastTrackGiftDlg::InitProcessManagerSupplyThreadData(WPARAM wparam,LPARAM lparam)
{
...
//
//
void CFastTrackGiftDlg::OnBnClickedProcessButton()
{
...
//
//
void CFastTrackGiftDlg::SpoofEntriesRetrievalStarted()
{
...
//
//
LRESULT CFastTrackGiftDlg::UpdateFileSharingManagerSupplyProjects(WPARAM wparam,LPARAM lparam)
{
	...
	p_parent->UpdateFileSharingManagerSupplyProjects(wparam,lparam);
...
//
//
LRESULT CFastTrackGiftDlg::FileSharingManagerThreadProject(WPARAM wparam,LPARAM lparam)
{
...
void CFastTrackGiftDlg::UpdateSpoofSize(UINT spoof_size, UINT temp_spoof_size, UINT spoof_index, UINT round, UINT hash_size)
{
	...
	sprintf(buf,"Temp Spoof Entries Size: %u", temp_spoof_size);
...
//
//
void CFastTrackGiftDlg::OnBnClickedPauseButton()
{
...
//
//
void CFastTrackGiftDlg::AutoPause(bool pause)
{
	if(pause)
...
//
//
void CFastTrackGiftDlg::OnBnClickedApplyAutoPauseThresholdButton()
{
	if(UpdateData())
...
//
//
LRESULT CFastTrackGiftDlg::FileSharingManagerSupplyRetrievalFailed(WPARAM wparam,LPARAM lparam)
{
	CString str = (CTime::GetCurrentTime()).Format("Last Spoof Entries Retrieval Failed at %Y-%m-%d %H:%M:%S");
	GetDlgItem(IDC_RETRIEVAL_PROCESS_ENDED_STATIC)->SetWindowText(str);
	p_parent->FileSharingManagerSupplyRetrievalFailed();
	return 0;
}
