...
//	m_ready_to_write_to_demand_db=false;
	...
	while(!connected)
	{
		connected = g_supply_db_interface.OpenSupplyConnection("127.0.0.1","onsystems","ebertsux37","fasttrack_raw_supply");
...
{
#ifdef HASH_GROWTH
	DBManager::g_supply_db_interface.InsertDistinctFastTrackSupply(*project_query_hit,today_date);
#else
	DBManager::g_supply_db_interface.InsertRawFastTrackSupply(*project_query_hit);
#endif
}
...
{
#ifdef HASH_GROWTH
	DBManager::g_supply_db_interface.InsertDistinctFastTrackSupply(project_query_hits,today_date);
#else
	DBManager::g_supply_db_interface.InsertRawFastTrackSupply(project_query_hits);
#endif
}
...
UINT RawDataFromFileToDBThreadProc(LPVOID pParam)
{
	...
	while(!connected)
	{
		connected = db_interface.OpenSupplyConnection("127.0.0.1","onsystems","ebertsux37","fasttrack_raw_supply");
		Sleep(1000);
	}
...
#ifdef HASH_GROWTH
					db_interface.InsertDistinctFastTrackSupply(query_hits,today_date_string);
#else
					db_interface.InsertRawFastTrackSupply(query_hits);
#endif
				}
			}
			delete [] buf;
...
//
//
void DBManager::InitParent(FastTrackGiftDll *parent)
{
