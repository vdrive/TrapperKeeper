...
//
//
void SupplyManager::OnFiveSecondTimer()
{
	if(!m_auto_pause_searching && !m_manual_pause_searching && v_supply_projects.size() > 0)
	{
//#ifndef HASH_GROWTH
#ifdef DATA_COLLECTION_ENABLED
		...
		if(p_parent != NULL)
		{
			...
			{
				if(/*p_connection_manager->ReturnModCount()>=p_parent->GetMinModuleCount() && */p_connection_manager->ReadyToSearch())
				{
					if(m_search_counts==20 && m_reconnected==false) //reconnect every 20 projects
					{
						if(p_parent->ReadyToReconnect())
						{
							m_reconnected = true;
		...
		if(!m_auto_pause_searching && !m_manual_pause_searching && v_supply_projects.size() > 0)
		{
			if(p_parent!=NULL)
			{
				if(p_parent->IsDBMaintaining() == false && p_connection_manager->ReadyToSearch())
				{
					for(UINT i=0; i<v_supply_projects.size(); i++)
					{
						if(m_second_counter % (v_supply_projects[i].m_interval * 60 * m_supply_interval_multiplier) ==  (v_supply_projects[i].m_offset * m_supply_interval_multiplier))
						{
							//we need to reconnect to all supernodes when we search for the same project again
							//otherwise, we will get way less result than what supposed to be
							/*
							if(i==0 && !m_first_round_searching)
							{
								m_first_round_searching = true;
								m_5_mins_passed = false;
								m_second_counter = 0;
							...
							*/
							...
							m_reconnected=false;
							p_parent->StopGatheringProjectData(v_supply_projects[i].m_name);
							v_supply_projects[i].m_search_id = p_connection_manager->PerformProjectSupplyQuery((char *)v_supply_projects[i].m_name.c_str(),v_supply_projects[i].m_project_id);
							m_first_round_searching = false;
							m_search_counts++;
						}
					}
					m_second_counter++;
		...
		{
			if(p_connection_manager->ReturnModCount()>=p_parent->GetMinModuleCount() && m_second_counter >= 180)
			{
				m_5_mins_passed = true;
				m_second_counter = 0;
			}
			else if(p_connection_manager->ReturnModCount()>=p_parent->GetMinModuleCount())
				m_second_counter++;
		}
		*/
#endif
//#else
		...
		if(m_5_mins_passed)
		{
			if(p_parent!=NULL)
			{
				if(p_parent->IsDBMaintaining() == false)
				{
					if(m_second_counter % 3600 == 0 && m_second_counter != 0) //reconnect all supernodes every hour
					{
						m_5_mins_passed = false;
						m_second_counter = 0;
						p_connection_manager->ReConnectAll();
					...
					else if(m_second_counter % 300 == 0) //do a new search every 5 mins
					{
						for(UINT i=0; i<v_supply_projects.size(); i++)
						{
							p_parent->StopGatheringProjectData(v_supply_projects[i].m_name);
							v_supply_projects[i].m_search_id = p_connection_manager->PerformProjectSupplyQuery((char *)v_supply_projects[i].m_name.c_str());
						}
					}
					m_second_counter++;
				}
