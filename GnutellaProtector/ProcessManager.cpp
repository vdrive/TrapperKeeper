...
//
//
void ProcessManager::InitParent(GnutellaProtectorDll *parent)
{
...
void ProcessManager::StartProcessRawData(DataBaseInfo& db_info, vector<ProjectKeywords>& keywords)
{
	if(!m_is_processing)
	{
		...
		if(strlen(db_info.m_db_host.c_str())!=0)
		{
											...
											db_info.m_db_password.c_str(),"DCDATA"))
			{
				...
				if(g_db_interface.OpenRawDataConnection("localhost","onsystems","ebertsux37","gnutella"))
...
//
//
void ProcessManager::GetProcessProgress(int& demand_progress)
{
...
void ProcessManager::DBMaintenance(void)
{
	if(!m_is_maintaining)
	{
		...
		{
			//db connection to raw data database
			connected = g_db_interface.OpenRawDataConnection("localhost","onsystems","ebertsux37","gnutella");
			trial++;
			if(trial > 10)
				break;
			Sleep(500);
