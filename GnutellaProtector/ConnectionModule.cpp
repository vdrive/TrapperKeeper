...
#include "GnutellaProtectorDll.h"	// temp kludge for logging
...
//	int i;
	...
	sockets.p_supply = thread_data.p_supply;
	...
	while(1)
	{
		...
		if(event==3)
		{
			...
			int compression_on_counts = 0;

#ifdef COMPRESSION_ENABLED
			compression_on_counts = sockets.ReturnCompressionOnCounts();
#endif
			...
			PostMessage(hwnd,WM_VENDOR_COUNTS_READY,(WPARAM)tmp_vendor_counts,(LPARAM)compression_on_counts);
		}
...
{
	p_supply = p_manager->ReturnProjectSupplyPointer();
...
void ConnectionModule::InitThreadData(WPARAM wparam,LPARAM lparam)
{
	...
	((ConnectionModuleThreadData *)lparam)->p_supply = p_supply;
...
{
	p_manager->VendorCountsReady((vector<VendorCount> *)wparam,(int)lparam);
}
