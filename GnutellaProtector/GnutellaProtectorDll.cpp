...
GnutellaProtectorDll::GnutellaProtectorDll(void)
{
}
...
//
//
void GnutellaProtectorDll::DllInitialize()
{
	/*
	...
	m_dlg.Create(IDD_GNUTELLA_PROTECTOR_DIALOG,CWnd::GetDesktopWindow());
...
//
//
void GnutellaProtectorDll::DllUnInitialize()
{
...
//
//
void GnutellaProtectorDll::DllStart()
{
...
#ifdef DC2
#ifdef _DEBUG
	m_min_module_count=m_max_module_count=5;
#else
	m_min_module_count=m_max_module_count=50;
#endif
#else
#ifdef _DEBUG
	m_min_module_count=m_max_module_count=5;
	...
	m_gnutella_raw_db_info = m_processed_db_info;
#ifdef DC2	
	m_gnutella_raw_db_info.m_db_host = "63.216.246.43";
#else
	m_gnutella_raw_db_info.m_db_host = "63.221.232.35";
#endif
	m_connection_manager.ReadInDecoyHashes(m_gnutella_raw_db_info);
	...
	m_syncher.Register(this, "GnutellaHostCache");
...
//
//
void GnutellaProtectorDll::DllShowGUI()
{
...
//
//
void GnutellaProtectorDll::DataReceived(char *source_name, void *data, int data_length)
{
	...
	switch (header->op)
	{
...
#ifdef SPLITTED
#ifdef BEARSHARE_ONLY
			header.op = DCHeader::BearShare_Protector_Init_Response;
#else
			header.op = DCHeader::Gnutella_Spoofer_Spited_Init_Response;
#endif
#else
			header.op = DCHeader::Gnutella_Protector_Init_Response;
#endif
			header.size=0;
		...
		*/
		...
		case DCHeader::GnutellaRawDatabaseInfo:
		{
			m_gnutella_raw_db_info.ReadFromBuffer(ptr);
#ifdef BEARSHARE_ONLY
			m_gnutella_raw_db_info.m_db_host="205.134.239.3";
#endif			
			m_supply_manager.UpdateGnutellaRawDBInfo(m_gnutella_raw_db_info);
			//m_connection_manager.ReadInDecoyHashes(m_gnutella_raw_db_info);
			break;
		}
...
//
//
void GnutellaProtectorDll::ReportConnectionStatus(ConnectionModuleStatusData &status)
{
...
//	m_logfile_manager.Log(status);
	...
	{
		if(status.v_project_status[i].v_queries.size()>0)
...
//
//
void GnutellaProtectorDll::VendorCountsReady(vector<VendorCount>& vendor_counts,int compression_on_counts)
{
	m_dlg.UpdateVendorCounts(vendor_counts,compression_on_counts);
}

//
//
//
void GnutellaProtectorDll::Log(char* msg)
{
...
//
//
void GnutellaProtectorDll::RequestProjectUpdates(char* dc_master, ProjectChecksums &projects_requiring_update)
{
...
//
//
void GnutellaProtectorDll::KeywordsUpdated()
{
	m_dlg.KeywordsUpdated(m_keyword_manager.v_project_keywords);
	m_supply_manager.KeywordsUpdated();

	// Send all partial project status report data
	//SendAllPartialProjectStatusReportData();
}
...
//
/*
void GnutellaProtectorDll::UpdateSupplyProjects(vector<SupplyProject> &supply_projects)
{
...
//
//
void GnutellaProtectorDll::StopGatheringProjectData(string &project)
{
...
//
//
void GnutellaProtectorDll::ShowLogWindow()
{
...
//
//
void GnutellaProtectorDll::OnTimer(UINT nIDEvent)
{
	UINT timer_ret=1;
...
//			m_search_manager.TimerHasFired();
			...
			{
				int demand=0;
				m_process_manager.GetProcessProgress(demand);
				m_dlg.UpdateDataProcessProgressBar(demand);
			}
#endif
		...
		{
			m_supply_manager.OnOneMinuteTimer();
...
//
/*
void GnutellaProtectorDll::SupplyManagerThreadDataReady(WPARAM wparam,LPARAM lparam)
{
...
//
//
void GnutellaProtectorDll::InitLogfileManagerThreadData(WPARAM wparam,LPARAM lparam)
{
...
//
//
void GnutellaProtectorDll::InitDBManagerThreadData(WPARAM wparam,LPARAM lparam)
{
	m_db_manager.InitThreadData(wparam,lparam);
}

//
//
//
int GnutellaProtectorDll::AlterModuleCounts(int min,int max)
{
...
//
//
void GnutellaProtectorDll::ReadyToWriteDataToDatabase(WPARAM wparam,LPARAM lparam)
{
	m_db_manager.ReadyToWriteDataToDatabase(wparam,lparam);
}
...
//
/*
void GnutellaProtectorDll::ExtractProjectStatusReportData(ConnectionModuleStatusData &status)
{
	UINT i,j;
...
//
//
void GnutellaProtectorDll::SendAllPartialProjectStatusReportData()
{
	// For all of the project status report data we have, send them all
/* =>_<=
	while(v_project_status_report_data.size()>0)
	{
		StopGatheringProjectData(v_project_status_report_data[0].m_project_name);
	}
 =>_= */
}
...
//
//
void GnutellaProtectorDll::InitPerformanceCounters()
{
	UINT timer_ret=1;
...
//
//
void GnutellaProtectorDll::CheckPerformanceCounters()
{
	UINT i;
...
//		m_dlg.SetWindowText("Error formatting pdh coutner data");
	...
	// If less than 60 % kick off another mod if there is less than min count of them
	if((m_proc_percent_usage<60) & (m_connection_manager.ReturnModCount()<(UINT)m_min_module_count))
	{
...
//		MessageBox(NULL,m_keynames[1].c_str(),"On this network adaptor",MB_OK);
	...
	//Get DB queues sizes
	UINT queries_size;
	m_db_manager.ReportDBStatus(queries_size);
...
//
//
void GnutellaProtectorDll::ClosePerformanceCounters()
{
	 PdhCloseQuery(m_pdh);
}
...
//
//
void GnutellaProtectorDll::SendStatus()
{
/*	int i;
...
//
//
void GnutellaProtectorDll::CheckModuleConnections()
{
	UINT i;
...
//
//
void GnutellaProtectorDll::InitProcessManagerThreadData(WPARAM wparam,LPARAM lparam)
{
	m_process_manager.InitThreadData(wparam, lparam);
}
...
//
//
void GnutellaProtectorDll::ProcessManagerDone(WPARAM wparam,LPARAM lparam)
{
	m_process_manager.ProcessFinished();
}

//
//
//
void GnutellaProtectorDll::StartDataProcess()
{
	m_process_manager.StartProcessRawData(m_processed_db_info, m_keyword_manager.v_project_keywords);
}
...
//
/*
void GnutellaProtectorDll::InitDBManagerMaintenanceThreadData(WPARAM wparam,LPARAM lparam)
{
...
//
//
void GnutellaProtectorDll::ProcessManagerMaintenanceDone(WPARAM wparam,LPARAM lparam)
{
...
//
//
void GnutellaProtectorDll::DBMaintenanceReadyToStart(void)
{
...
//
//
void GnutellaProtectorDll::DBMaintenanceFinished(void)
{
...
//
//
void GnutellaProtectorDll::SpoofEntriesRetrievalStarted()
{
...
//
//
void GnutellaProtectorDll::InitSupplyManagerThreadData(WPARAM wparam,LPARAM lparam)
{
...
//
//
void GnutellaProtectorDll::UpdateSupplyManagerSupplyProjects(WPARAM wparam,LPARAM lparam)
{
	m_supply_manager.UpdateSupplyManagerSupplyProjects(wparam,lparam);
...
//
//
void GnutellaProtectorDll::RetrieveSupplyEntries()
{
	m_supply_manager.RetrieveSupplyEntriesWithZeroSize();
}

//
//
//
void GnutellaProtectorDll::AddHost(CString host)
{
	m_connection_manager.ConnectToHost(host);
}

//
//
//
void GnutellaProtectorDll::EnableVendorCounts(BOOL enabled)
{
	(bool)m_connection_manager.m_vendor_counts_enabled=enabled;
}

//
//
//
void GnutellaProtectorDll::SendSupplyRetrievalStartingTime()
{
...
//
//
void GnutellaProtectorDll::SendSupplyRetrievalEndingTime()
{
	int buf_len = sizeof(DCHeader);
	unsigned char* buf = new unsigned char[buf_len];
...
//
//
void GnutellaProtectorDll::SendSupplyRetrievalProgress(int progress)
{
