...
{
	v_project_keywords.clear();
	//find out all the music projects
	for(UINT i=0; i<v_all_project_keywords.size();i++)
	{
		if(v_all_project_keywords[i].m_project_active == true)
		{
			//if(v_all_project_keywords[i].m_supply_keywords.v_keywords.size() > 0)
			if(v_all_project_keywords[i].m_piolet_supply_enabled == true || v_all_project_keywords[i].m_piolet_demand_enabled == true)
...
//	p_search_manager->KeywordsUpdated();
	...
	for(i=0; i<v_project_keywords.size();i++)
	{
		if(v_project_keywords[i].m_piolet_supply_enabled)
		{
