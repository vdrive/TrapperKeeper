...
MovieMakerModule DllInterface::m_the_app;
...
//
//
bool DllInterface::ReceivedData(AppID from_app_id, void* data, UINT datasize)
{
	bool ret = m_the_app.DllReceivedData(from_app_id, data, datasize);
	return ret;
...
AppID DllInterface::GetAppID()
{
	...
	m_app_id.m_app_id = 37;
	m_app_id.m_version = 0x00000001;
	m_app_id.m_app_name = "Movie Maker";
	return m_app_id;
}
...
//
//
bool DllInterface::SendData(AppID to_app_id, void *data, UINT datasize)
{
	typedef bool (*MYPROC)(AppID, AppID, void*, UINT);
	MYPROC ProcAdd;

	ProcAdd = (MYPROC) GetProcAddress(m_hinst, "DllManagerSendData");

	return (ProcAdd)(m_app_id, to_app_id, data, datasize);
}
