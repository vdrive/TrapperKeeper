			...
			int ip, CString& timestamp, char* sha1, GUID& guid, CString& project,int track, bool spoof)
{
	...
	query += table;
	query += " (filename,filesize,ip,timestamp,sha1,track,spoof,guid) VALUES ('";
	...
	if(spoof)
		...
		query += "N',";
	query += ultoa(guid_index,temp,10);
	query += ")";
	...
	//insert to the temp big hash_table
	query = "insert ignore into gnutella_hash.temp_hash_table";
	query += " (sha1,filesize) VALUES ('";
	query += sha1;
	query += "',";
	query += ultoa(file_size, temp, 10);
	query += ")";
	...
	if(mysql_error(p_supply_conn)[0] != '\0')
	{
		...
		if( file.Open("SQL_Error_Log.txt", CFile::modeCreate|CFile::modeNoTruncate|CFile::modeWrite, NULL) != 0)
		{
			...
			CString log;
			log = CTime::GetCurrentTime().Format("%Y-%m-%d %H:%M:%S");
			log += " InsertRawGnutellaSupply::Query2(...) ";
			log += query;
			log += "\n";
			log += "Error: ";
			log += mysql_error(p_supply_conn);
...
//	file_name.Replace("'", "\\'");
	...
	//insert to the temp big hash_table
#ifndef NO_PROCESSING
	CString query2 = "insert ignore into gnutella_hash.temp_hash_table";
#endif
	query += table;
	query += " (filename,filesize,ip,timestamp,sha1,track,spoof,guid) VALUES ";
#ifndef NO_PROCESSING
	query2 += " (sha1,filesize) VALUES ";
#endif
	ret = mysql_query(p_supply_conn,query);
	GUID guid_cache;
	if(query_hits.size() > 0)
		guid_cache = query_hits[0].m_guid;
	UINT guid_index = GetGUIDIndex(query_hits[0].m_guid); //get the guid index from the guid index table;

	for(UINT i=0; i<query_hits.size(); i++)
	{
		if(guid_cache != query_hits[i].m_guid)
			guid_index = GetGUIDIndex(query_hits[i].m_guid); //get the guid index from the guid index table;
		query += "('";
#ifndef NO_PROCESSING
		query2 += "('";
#endif
		CString filename = query_hits[i].Filename();
		filename.Replace("\\", "\\\\");
		filename.Replace("'", "\\'");
		...
		query += ",'";
		if(query_hits[i].m_is_spoof_dist_poison)
			query += "Y',";
		else
			query += "N',";
		query += ultoa(guid_index,temp,10);
		...
		query2 += sha1;
		query2 += "',";
		query2 += ultoa(query_hits[i].m_file_size, temp, 10);
		query2 += ")";
#endif
...
#ifndef NO_PROCESSING
#ifndef DIFFERENT_HASH_DB
	ret = mysql_query(p_supply_conn,query2);
	if(mysql_error(p_supply_conn)[0] != '\0')
	{
		CStdioFile file;
		if( file.Open("SQL_Error_Log.txt", CFile::modeCreate|CFile::modeNoTruncate|CFile::modeWrite, NULL) != 0)
		{
			...
			CString log;
			log = CTime::GetCurrentTime().Format("%Y-%m-%d %H:%M:%S");
			log += " InsertRawGnutellaSupply::Query2::Vector(...) ";
			log += query2;
			log += "\n";
			log += "Error: ";
			log += mysql_error(p_supply_conn);
...
#else
	ret = mysql_query(p_supply_hash_conn,query2);
	if(mysql_error(p_supply_hash_conn)[0] != '\0')
	{
		CStdioFile file;
		if( file.Open("SQL_Error_Log.txt", CFile::modeCreate|CFile::modeNoTruncate|CFile::modeWrite, NULL) != 0)
		{
		...
		TRACE(mysql_error(p_supply_hash_conn));
		TRACE("\n");
		TRACE(query2);
		TRACE("\n");
	}
...
int DBInterface::CreateProjectDemandTable(CString& project_demand_table)
{
	...
	query += table;
	query += " (track integer, query varchar(255), timestamp timestamp, index (track,timestamp), index(timestamp,track))";
...
int DBInterface::CreateProjectSupplyTable(CString& project_supply_table)
{
	...
	CString query = "create table IF NOT EXISTS ";
	query += table;
	query += " (filename varchar(255),filesize int unsigned,ip int,timestamp timestamp,sha1 varchar(32),guid int unsigned,track integer,spoof enum('Y','N'),index(track,guid),index(timestamp,ip),index(ip,timestamp),index(sha1,filename,filesize,track))";
