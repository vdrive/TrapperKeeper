...
{
	HKEY hKey;
	char subkey[]="Software\\MediaDefender\\GnutellaSupplyDemand\\Module Count Limits";
...
{
	HKEY hKey;
	char subkey[]="Software\\MediaDefender\\GnutellaSupplyDemand\\Supply Interval Multiplier";
	RegCreateKeyEx(HKEY_CURRENT_USER,subkey,0,NULL,REG_OPTION_NON_VOLATILE,KEY_WRITE,NULL,&hKey,NULL);
...
//
//
void RegistryKeys::SetLastSearchingProjectIndex(UINT index)
{
	HKEY hKey;
	char subkey[]="Software\\MediaDefender\\GnutellaSupply\\Last Search Index";
	RegCreateKeyEx(HKEY_CURRENT_USER,subkey,0,NULL,REG_OPTION_NON_VOLATILE,KEY_WRITE,NULL,&hKey,NULL);

	DWORD cbData=sizeof(UINT);
	RegSetValueEx(hKey,"index",0,REG_DWORD,(unsigned char *)&index,cbData);
	RegCloseKey(hKey);
}
...
//
//
void RegistryKeys::GetLastSearchingProjectIndex(UINT *index)
{
	// If this is the first time running, or the key doesn't exist, return default values
	*index=0;
	...
	if(RegOpenKeyEx(HKEY_CURRENT_USER,subkey,0,KEY_READ,&hKey)==ERROR_SUCCESS)
	{
		...
		DWORD i=0;
		while(RegEnumValue(hKey,i,szName,&cbName,NULL,&dwType,(unsigned char *)&val,&cbData)==ERROR_SUCCESS)
		{
			if(strcmp(szName,"index")==0)
			{
				*index=val;
			}
			cbName=sizeof(szName)/sizeof(szName[0]);	// reset the size
