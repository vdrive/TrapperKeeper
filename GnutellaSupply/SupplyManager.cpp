...
void SupplyManager::OnOneSecondTimer()
{
	if(m_5_mins_passed)
	{
		if(p_parent!=NULL)
		{
			if( (p_parent->IsDBMaintaining() == false) && (p_parent->ReadyToSearch()))
			{
				...
				m_second_counter++;
				if(v_supply_projects.size())
				{
					RegistryKeys::SetLastSearchingProjectIndex(m_second_counter);
				}
			}
		}
	}
	...
	{
#ifdef _DEBUG
		if(m_second_counter >= 60)
		{
			m_5_mins_passed = true;
			RegistryKeys reg;
			if(v_supply_projects.size())
				reg.GetLastSearchingProjectIndex(&m_second_counter);
		}
...
//#else
			...
			if(v_supply_projects.size())
				RegistryKeys::GetLastSearchingProjectIndex(&m_second_counter);
