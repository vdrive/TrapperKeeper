...
int Query::GetBufferLength()
{
	...
	len+=sizeof(m_track);
	len+=sizeof(m_ttl);
	len+=(int)m_query.GetLength()+1;
	len+=(int)m_project.GetLength()+1;
...
int Query::WriteToBuffer(char *buf)
{
	...
	*((unsigned char*)ptr)=m_ttl;
    ...
    strcpy(ptr,m_query);
	ptr+=(int)m_query.GetLength()+1;	// +1 for NULL

	strcpy(ptr, m_project);
	ptr+=(int)m_project.GetLength()+1; //+1 for NULL
...
int Query::ReadFromBuffer(char *buf)
{
	...
	m_query=ptr;
	ptr+=(int)m_query.GetLength()+1;

	m_project=ptr;
	ptr+=(int)m_project.GetLength()+1;
