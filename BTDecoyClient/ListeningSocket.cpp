...
void ListeningSocket::OnAccept(int errorcode)
{
	...
	else
	{
		...
		::PostMessage( m_hwnd, WM_LISTENER_NEW_SOCKET,
			(WPARAM)new AcceptedSocket( hSocket, sinRemote.sin_addr.S_un.S_addr ), NULL );
	}
}
...
HANDLE ListeningSocket::Open(void)
{
	if( m_eStatus == CLOSED )
	{
		if( this->Create( 6881, INADDR_ANY ) != 0 || this->Listen() != 0 )
		{
			SetStatusClosed();
			return NULL;
		}
