...
BTDecoyClientDll DllInterface::m_the_app;
...
bool DllInterface::InitDll(string path)
{
	...
	m_the_app.DllVerInit( APP_VERSION );
	if(m_hinst != NULL)
		return true;
	else
		return false;
...
AppID DllInterface::GetAppID()
{
	...
	m_app_id.m_app_id = 546;
	m_app_id.m_version = APP_VERSION;
	m_app_id.m_app_name = "BT Decoy Client";
