...
void WriteProjectSpoofToDatabase(Query *project_queries)
{
	CString timestamp = project_queries->m_timestamp.Format("%Y%m%d%H%M%S");
//	CString filename = project_query_hits->Filename();
	DBManager::g_db_interface.InsertRawPioletSpoof(PIOLET_SPOOF_TABLE,project_queries->m_project.c_str(), project_queries,timestamp);
}
...
UINT RawDataFromFileToDBThreadProc(LPVOID pParam)
{
	...
	if(raw_queries_file.Open("raw_queries.dat",CFile::modeRead|CFile::typeBinary
		|CFile::shareDenyRead|CFile::shareDenyWrite) != 0)
	{
		...
		while(raw_queries_file.Read(&data_size, sizeof(UINT)))
		{
			...
			db_interface.InsertRawPioletSpoof(PIOLET_SPOOF_TABLE, query.m_project.c_str(),&query, timestamp);
			delete [] buf;
		}
		raw_queries_file.Close();
		remove("raw_queries.dat");
	}
...
UINT DBManagerThreadProc(LPVOID pParam)
{
	...
	while(1)
	{
		...
		if(project_queries_cache.size()>0)
		{
			...
			{
				WriteProjectSpoofToDatabase(project_queries_cache[i]);
...
//
//
void DBManager::InitParent(PioletSpooferDll *parent)
{
...
//
//
void DBManager::PushSpoofData(vector<Query> &queries)
{
#ifndef NO_PROCESSING
	...
	{
		CFile raw_queries_file;
		if(raw_queries_file.Open("raw_queries.dat",CFile::modeWrite|CFile::modeCreate|CFile::modeNoTruncate|CFile::typeBinary
			|CFile::shareDenyWrite|CFile::shareDenyRead) != 0)
		{
			raw_queries_file.SeekToEnd();
			for(UINT i=0;i<queries.size();i++)
			{
				UINT data_size = queries[i].GetBufferLength();
				raw_queries_file.Write(&data_size, sizeof(UINT));
				char* buf = new char[data_size];
				queries[i].WriteToBuffer(buf);
...
void DBManager::WriteDataToDatabase()
{
	...
	// If there are no project queries to send to the thread to be written to database, then return
