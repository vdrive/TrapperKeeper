...
//
//
void ProcessManager::InitParent(PioletSpooferDll *parent)
{
...
UINT ProcessManagerThreadProc(LPVOID pParam)
{
	...
	while(1)
	{
		...
		if(event==1)
		{
			...
			{
				thread_data.m_spoof_progress=0;

				//process raw demand data
				for(i=0; i<(UINT)thread_data.p_projects->size(); i++)
				{
					::PostMessage(hwnd, WM_SPOOF_PROCESS_PROJECT, (WPARAM)(*thread_data.p_projects)[i].m_project_name.c_str(),0);
					char date[32];
					...
					while(more_demand)
					{
						...
						//check to see if we have reached today, then stop spoof process for this project
						...
						//get spoofs and insert to processed database
						char on_date[32];
					...
					thread_data.m_spoof_progress = (int)((float)i/(float)thread_data.p_projects->size()*(float)100);
...
UINT DBMaintenanceThreadProc(LPVOID pParam)
{
	...
	UINT spoofs_records_delete = 0;

	spoofs_records_delete = ProcessManager::g_db_interface.DeleteRawData(delete_timestamp, PIOLET_SPOOF_TABLE);
	vector<CString>tables;
	tables.push_back(PIOLET_SPOOF_TABLE);
				
	//optimize and reclaim the unused space
	ProcessManager::g_db_interface.OptimizeTables(tables);
	
	::PostMessage(hwnd, WM_PROCESST_MANAGER_DB_MAINTENANCE_DONE,spoofs_records_delete,0);
	return 0;	// exit the thread
}
	...
	if(!m_is_processing)
	{
										...
										db_info.m_db_password.c_str(),"DCDATA"))
		{
			...
			if(g_db_interface.OpenRawDataConnection("localhost","onsystems","ebertsux37","piolet"))
				AfxBeginThread(ProcessManagerThreadProc,(LPVOID)p_parent->m_dlg.GetSafeHwnd(),THREAD_PRIORITY_BELOW_NORMAL);
			else
				m_is_processing = false;
		}
		else
...
//
//
void ProcessManager::GetProcessProgress(int& spoof_progress)
{
	if(p_thread_data != NULL)
	{
		spoof_progress = p_thread_data->m_spoof_progress;
	}
}
...
void ProcessManager::DBMaintenance(void)
{
	if(!m_is_maintaining)
	{
		...
		*/
		...
		{
			p_parent->DBMaintenanceReadyToStart();
			AfxBeginThread(DBMaintenanceThreadProc,(LPVOID)p_parent->m_dlg.GetSafeHwnd(),THREAD_PRIORITY_BELOW_NORMAL);
		}
		else
			m_is_maintaining = false;
	}
