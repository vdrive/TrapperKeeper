...
//	m_socket.p_supply_projects = thread_data.p_supply;
...
ConnectionModule::ConnectionModule()
{
	...
	AfxBeginThread(ConnectionModuleThreadProc,(LPVOID)this,THREAD_PRIORITY_NORMAL);
}
...
void ConnectionModule::ProjectSupplyUpdated(char* project)
{
	...
	*/
	...
	p_supply = p_manager->ReturnSupplyProjectsPointer();
...
//		p_manager->p_parent->m_log_window_manager.Log("ConnectionModule::ProjectSupplyUpdated() - 1 - Before LOCK\n",0,false,true);	// black italic
		...
		if(singleLock.IsLocked())
		{
			m_thread_data.p_supply=p_supply;
...
void ConnectionModule::PerformProjectSupplyQuery(ProjectSupplyQuery &psq)
{
	...
	while(psq_iter!=v_project_supply_queries.end())
	{
		if((strcmp(psq_iter->m_search_string.c_str(),psq.m_search_string.c_str())==0) /* =>_<= && (psq_iter->m_is_searcher_query==psq.m_is_searcher_query)*/)
		{
			v_project_supply_queries.erase(psq_iter);
			break;
		}
...
//
//
void ConnectionModule::SendPoisonEntry(char* poisoner, PoisonEntries& pe)
{
	p_manager->SendPoisonEntry(poisoner, pe);
}

//
//
//
vector<PoisonerStatus>* ConnectionModule::GetPoisoners()
{
	return p_manager->GetPoisoners();
}

//
//
//
int ConnectionModule::GetNumPoisoners()
{
	return p_manager->GetNumPoisoners();
}
...
//
/*
vector<SupplyProject>* ConnectionModule::GetSupplyProjectPointer()
{
	p_manager->LockSupplyProject();
	return p_supply;
}
...
//
//
void ConnectionModule::ReleaseSupplyProjectPointer()
{
	p_manager->ReleaseSupplyProject();
}
*/
//
//
//
void ConnectionModule::Log(char *buf, COLORREF color, bool bold, bool italic)
{
	/*
	...
	*/
	p_manager->Log(buf, color, bold, italic);
}

//
//
//
bool ConnectionModule::IsProjectSupplyBeingUpdated()
{
	if(p_manager->p_supply_manager != NULL)
		return p_manager->p_supply_manager->IsSupplyProjectsBeingUpdated();
	else
		return true;
}
