...
#define MAX_SPOOF_ENTRY_PER_TRACK	200
#define MAX_POISON_ENTRY_PER_TRACK	50
...
UINT SupplyManagerThreadProc(LPVOID pParam)
{
	...
	while(1)
	{
		...
		// Get the latest supply entris from piolet raw data database
		if(event==1)
		{
...
//			{
				...
				if(thread_data.p_piolet_raw_db_info!=NULL)
				{
					if(strlen(thread_data.p_piolet_raw_db_info->m_db_host.c_str())!= 0)
					{
						int trial = 0;
						while(!connected && trial < 900) // keeping trying for 15 mins
						{
							connected = supply_entry_db_interface.OpenConnection(thread_data.p_piolet_raw_db_info->m_db_host.c_str(),
								thread_data.p_piolet_raw_db_info->m_db_user.c_str(), thread_data.p_piolet_raw_db_info->m_db_password.c_str(), "piolet");
							if(!connected)
							{
								Sleep(1000);
								trial++;
						...
						if(connected)
						{
							for(UINT i=0; i<supply_projects.size(); i++)
							{
								...
								supply_projects[i].v_poison_entries.clear();
								PostMessage(hwnd,WM_SUPPLY_MANAGER_THREAD_PROJECT,(WPARAM)supply_projects[i].m_name.c_str(),0);
								supply_entry_db_interface.RetrieveSupplyEntry(supply_projects[i],MAX_SPOOF_ENTRY_PER_TRACK);
//#ifdef PIOLET_POISON
								supply_entry_db_interface.RetrievePoisonEntry(supply_projects[i],MAX_POISON_ENTRY_PER_TRACK);
//#endif
...
//#ifdef PIOLET_POISON
								supply_projects[i].v_poison_entries.clear();
//#endif
								PostMessage(hwnd,WM_SUPPLY_MANAGER_THREAD_PROGRESS, (WPARAM)i, (LPARAM)supply_projects.size());
							}
							supply_projects.clear();
							PostMessage(hwnd,WM_SUPPLY_MANAGER_THREAD_PROGRESS, (WPARAM)supply_projects.size(), (LPARAM)supply_projects.size());
...
//			}
			if(connected)
				PostMessage(hwnd,WM_UPDATE_SUPPLY_MANAGER_SUPPLY_PROJECTS,(WPARAM)0,(LPARAM)0);
		}
...
//
//
void SupplyManager::InitParent(PioletSpooferDll* parent)
{
	p_parent = parent;
	AfxBeginThread(SupplyManagerThreadProc,(LPVOID)p_parent->m_dlg.GetSafeHwnd(),THREAD_PRIORITY_BELOW_NORMAL);
...
void SupplyManager::InitThreadData(WPARAM wparam,LPARAM lparam)
{
	...
	p_thread_data->p_piolet_raw_db_info = &m_piolet_raw_db_info;
}
...
{
/*
	if(strlen(m_piolet_raw_db_info.m_db_host.c_str())>0)
	{
		if(m_minute_counter % 1440 == 0) //get the spoof entries once a day
		{
			if(p_critical_section!=NULL)
			{
...
{
#ifndef NO_PROCESSING
	char *folder="C:\\syncher\\src\\Piolet Supply Files\\";
#else
	char *folder="C:\\syncher\\rcv\\Piolet Supply Files\\";
#endif
	string path;
...
//
//
void SupplyManager::UpdatePioletRawDBInfo(DataBaseInfo& raw_db)
{
	m_piolet_raw_db_info = raw_db;
}
...
//
/*
void SupplyManager::LockSupplyProject()
{
	CSingleLock singleLock(&m_critical_section);
	if(!singleLock.IsLocked())
		singleLock.Lock();
...
//
//
void SupplyManager::ReleaseSupplyProject()
{
	CSingleLock singleLock(&m_critical_section);
	singleLock.Unlock();
}
