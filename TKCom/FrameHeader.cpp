...
//
//
FrameHeader::FrameHeader(unsigned int bitrate,double sampling_rate)
{
	Clear();
	Init();
	...
	{
		m_data[2]&=0x0F;	// clear it
		m_data[2]|=0x10;	// reset it
	}

	if(sampling_rate==32)
	{
		m_data[2]&=0xF3;	// clear it
		m_data[2]|=0x08;	// reset it
	}
}
...
void FrameHeader::Init()
{
	...
	//m_data[3]|=0x00;
	// Set Single Channel Mono	xxxx xxxx : xxxx xxxx :: xxxx xxxx : 11xx xxxx
	m_data[3]|=0xC0;
...
int FrameHeader::ExtractHeader(CFile *file)
{
	...
	if(file->Read(this,sizeof(FrameHeader))!=sizeof(FrameHeader))
	{
		return -1;
	}
