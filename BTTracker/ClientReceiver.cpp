...
//
//
void CClientReceiver::Init(BTTrackerDll *pParent)
{
	m_pParent = pParent;
}
...
void CClientReceiver::SetOpen(void)
{
	if( m_pCriticalSection != NULL && m_pThreadData != NULL )
	{
		...
		*(m_pThreadData->m_peStatus) = ListeningSocket::OPEN;
...
{
	m_pParent->AddNewClient( (SOCKET)wparam );
...
LRESULT CClientReceiver::LogMsg(WPARAM wparam, LPARAM lparam)
{
	if( wparam != NULL )
	{
		//m_pParent->CLogEntry( (char *)wparam );
		delete (char *)wparam;
	}
