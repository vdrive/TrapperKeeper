...
public:
	...
	HWND ExtractAutoSearch(HWND auto_dlg);
	void SetId(int id);
	void CloseKaZaA();
	void Clear();
	void NewSearch();
	...
	UINT LogResults(ThreadData *data);
	UINT SendFindMore(ThreadData *data);
	...
	HWND m_search_more_auto_check_hwnd;
	...
	DistributedKazaaCollectorDll *p_parent;
	...
	unsigned int m_search_more_interval;	// seconds
	...
	unsigned int m_number_of_projects_searched;
