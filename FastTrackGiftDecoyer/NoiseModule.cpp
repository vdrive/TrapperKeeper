...
*/
		...
		// Event 3 - change uploading files flag
		if(event==3)
		{
			WSAResetEvent(sockets.m_events[event]);	// reset event
...
//				sockets.SetBannedGUIDDeterminationValues(banned_guid_threshold,banned_guid_timer_length);
				sockets.m_sharing_enabled = thread_data.m_sharing_enabled;
				singleLock.Unlock();
			}
		}
...
//
//
NoiseModule::NoiseModule(CCriticalSection *connection_data_critical_section,vector<ConnectionData> *connection_data,
...
//
//
void NoiseModule::DisableSharing()
{
	m_sharing_enabled = false;
	if(p_critical_section!=NULL)
	{
		CSingleLock singleLock(p_critical_section);
		singleLock.Lock();
		if(singleLock.IsLocked())
		{
			p_thread_data->m_sharing_enabled = false;
			WSASetEvent(p_thread_data->m_reserved_events[3]);
			singleLock.Unlock();
		}
	}	
}
...
//
//
void NoiseModule::EnableSharing()
{
	...
	if(p_critical_section!=NULL)
	{
		...
		if(singleLock.IsLocked())
		{
			p_thread_data->m_sharing_enabled = true;
			WSASetEvent(p_thread_data->m_reserved_events[3]);
			singleLock.Unlock();
		}
	}	
