...
FSTCipher *fst_cipher_create()
{
	...
	cipher = (FSTCipher*)malloc (sizeof(FSTCipher));
...
static int pad_init (unsigned int *pseed, unsigned int enc_type, unsigned char* pad, unsigned int pad_size)
{
	...
	if ( (enc_type & 1) || !(enc_type & 0x1E7))
	{
		...
		EncryptionType1::enc_type_1 (key_256_out, key_256_in);
...
//			FST_WARN ("pad_init: enc_type & 0x04 not implemented");
		...
		if (enc_type & 0x20)
		{
			...
			seed = seed_step (seed);
			EncryptionType20::enc_type_20 (key_80, seed);
		}
		...
		if (enc_type & 0x80)
		{
			...
			seed = seed_step (seed);
			EncryptionType80::enc_type_80 (key_80, seed);
		}
