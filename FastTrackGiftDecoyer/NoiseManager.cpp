...
//	m_listening_port=2568;
#ifdef _DEBUG
	m_listening_port=1214;
#else
	m_listening_port=2597;
#endif
...
*/
	...
	NoiseModule *mod=new NoiseModule(&m_connection_data_critical_section,&v_connection_data,this,p_file_sharing_manager,m_sharing_enabled);
...
//
//
void NoiseManager::DisableSharing()
{
	m_sharing_enabled = false;
	for(UINT i=0;i<v_mods.size();i++)
	{
		v_mods[i]->DisableSharing();
	}
}
...
//
//
void NoiseManager::EnableSharing()
{
	m_sharing_enabled = true;
	for(UINT i=0;i<v_mods.size();i++)
	{
		v_mods[i]->EnableSharing();
	}
}