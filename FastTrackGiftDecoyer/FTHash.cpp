...
char * FTHash::HashEncode64()
{
	static char str[128];
	char *buf;

	if (!(buf = Base64Encode (m_data, FST_FTHASH_LEN)))
		return NULL;
...
char* FTHash::HashEncode16()
{
	static char str[128];
	char *buf;

	if (!(buf = HexEncode(m_data, FST_FTHASH_LEN)))
		return NULL;
...
char * FTHash::HexEncode (const unsigned char *data, int src_len)
{
...
BOOL FTHash::HashDecode16(const char *fthash)
{
	byte *buf;
	int len;
	memset(m_data,0,sizeof(m_data));
	
	if (!(buf = HexDecode (fthash, &len)))
		return FALSE;
	...
	memcpy(&m_data, buf, FST_FTHASH_LEN);
	free (buf);
...
//
/* caller frees returned string */
unsigned char * FTHash::HexDecode (const char *data, int *dst_len)
{
	static const char hex_string[] = "0123456789abcdefABCDEF";
	char *dst, *h;
	int i;
...
//
/* caller frees returned string */
char * FTHash::Base64Encode (const unsigned char *data, int src_len)
{
...
//
//
UINT FTHash::GetFileSize()
{
	UINT file_size = 0;
	file_size = (*(UINT*)&m_data[FST_FTHASH_LEN-4]);
	return file_size;
}