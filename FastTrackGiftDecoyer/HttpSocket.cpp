...
{
	if(p_sockets->m_sharing_enabled == false)
	{
...
//	char lfilename[sizeof(m_buf)];
	...
	*/
	...
	FileSizeAndHash fsh = p_sockets->p_fs_manager->GetFileSize(ft_hash.m_data);
	if(fsh.m_filesize==0)
	{
		OnClose(201);
		return;
...
//	m_status.m_filename=hash;
	...
	//UINT total_size = (p_sockets->p_fs_manager->GetFileSize(ft_hash.m_data)).m_filesize;

	m_status.m_total_size = fsh.m_filesize;
	m_status.m_filename = fsh.m_filename;
	...
	//if(fsh.m_filesize==0 || (fsh.m_filesize >= 31457280 && start > 31457280)) //30 MB //send busy signal
	//{
	//	string reply = "HTTP/1.1 503 Service Unavailable\r\n\r\n";
	//	unsigned char *data=new unsigned char[reply.size()+1];
	//	strcpy((char *)data,reply.c_str());
...
void HttpSocket::SocketDataSent(unsigned int len)
{
	...
	if(m_file_len==0 || p_sockets->m_sharing_enabled == false)
	{
		OnClose(0);
		return;
	}
